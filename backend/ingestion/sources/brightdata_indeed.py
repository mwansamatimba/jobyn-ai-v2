"""Bright Data Indeed Jobs discovery connector."""
...
class BrightDataIndeedConnector(JobSourceConnector):
    """Discover Indeed jobs through Bright Data's keyword dataset."""

    source_name = "indeed"
    source_type = "api"
    attribution_template = "Via Indeed"
    ...
    async def normalize_job(self, raw_job: dict[str, Any]) -> NormalizedJob | None:
        try:
            ...
            external_id = str(_first(raw_job, "jobid", "job_id", "id") or "").strip()
            location = str(_first(raw_job, "location", "job_location") or "").strip()
            description = str(
                _first(raw_job, "description_text", "description", "job_description") or ""
            )
            application_url = str(
                _first(raw_job, "application_url", "apply_url") or source_url
            ).strip()
            ...
            logger.warning("Malformed Indeed Bright Data record: %s", exc)
            return None
